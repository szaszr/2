...
/*
http://www.programmingbydoing.com/a/weekday-calculator.html
 */
...
public class WeekdayCalculator {
    ...
    public static String weekday( int mm, int dd, int yyyy ) {
        ...
        if (is_leap(yyyy) && mm==1 || mm==2){
            total = yy/4 + yy + month_offset(mm) + dd - 1;
        }
        else
            total = yy/4 + yy + month_offset(mm) + dd;

        int result = total%7;

        // bunch of calculations go here
        weekday_name(result);
        date = weekday_name(result) +", " + month_name(mm) + " " + dd + ", " + yyyy;
